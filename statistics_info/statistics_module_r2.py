...
def get_latest_three_day_data(monthly_key_list, sort_list):
    ...
    test_list = []
    for key in monthly_key_list:
        target_data = result_dict.get(key)
        test_list.append(target_data)
    
    three_date_list = [tuple(item.keys()) for item in gruopby_list]
    three_list = [[item[:3] + item[4:] for item in sorted(
        list(data.values())[0], key=lambda x: x[0])] for data in test_list]
    return three_date_list, three_list
...
def format_bill_info(three_date_list, total_sum_list, rows_list):
    ...
    format_rows_list = [
            {'month': data[0], 'all': tsum, 'detail': bill}
...
def create_dately_key_list():
    ...
    dately_key_list = []
    for i in range(1, 4):
        current_date = three_days_ago + timedelta(days=i)
        current_date_formatted = current_date.strftime('%Y/%m/%d')
        dately_key_list.append(current_date_formatted)
...
def create_monthly_key_list():
    ...
    monthly_key_list = []
    for i in range(3):
        current_month = current_datetime.month - i
        current_year = current_datetime.year
        if current_month <= 0:
            current_month += 12
            current_year -= 1
        current_month_first_day = datetime(current_year, current_month, 1)
        current_month_formatted = current_month_first_day.strftime('%Y/%m')
        monthly_key_list .append(current_month_formatted)
